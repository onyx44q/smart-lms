...
// Checks BOTH: courses.lecturer_id AND users.course_id (the assigned course on the lecturer record)
...
<body class="bg-slate-50 min-h-screen">
            ...
            <div class="w-full max-w-lg gen-panel rounded-3xl p-8 relative">
                ...
                <div id="gen-form" class="space-y-4">
                    <div>
                        ...
                        <select id="gen-course" class="w-full p-3.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm font-semibold outline-none focus:ring-2 focus:ring-indigo-500 appearance-none">
                    ...
                    <div>
                        <label class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1 block">Topic / Focus Area</label>
                        <input type="text" id="gen-topic" placeholder="e.g. Machine Learning Fundamentals"
    ...
    async function startGeneration() {
        const course_id     = document.getElementById('gen-course').value;
        ...
        try {
            ...
            // Show source badge in preview
            let sourceMsg = '';
            if (json.source === 'lecture_notes') {
                sourceMsg = `<span class="text-emerald-400"><i class="fa-solid fa-file-pdf mr-1"></i>AI generated from ${json.pdfs_read} PDF lecture note(s)</span>`;
            } else if (json.source === 'general_knowledge') {
                sourceMsg = `<span class="text-blue-400"><i class="fa-solid fa-brain mr-1"></i>AI generated from general knowledge</span>`;
            } else {
                sourceMsg = `<span class="text-amber-400"><i class="fa-solid fa-gear mr-1"></i>Generated by local engine — upload PDFs for AI questions</span>`;
            }
